          ...
          <Card
            ...
            className={`relative cursor-pointer transition-all ${
              selectedPack?.name === pack.name
                ...
                : "border-border hover:border-brand-amber/50"
            } ${pack.popular ? "ring-2 ring-brand-amber/50" : ""}`}
            onClick={() => !selectedPack && handleSelectPack(pack)}
          >
            {pack.popular && (
              <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-brand-amber text-black text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1">
            ...
            <CardFooter>
              <Button
                className="w-full"
                variant={selectedPack?.name === pack.name ? "amber" : "outline"}
